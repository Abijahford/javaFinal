...
public class objOrient extends JFrame implements ActionListener {  
    ...
    private String mode;
    private QuestionCollections questionObjOCollections;
    private Sub2 questionObjOPack;
    private int rightAnswer;
    
    objOrient(String s, String mode){   
        super(s);
        this.mode = mode;
        questionObjOPack = new Sub2(mode);
        questionObjOCollections = questionObjOPack.getCollections();
        l=new JLabel();  
        add(l);  
        bg=new ButtonGroup(); 
            ...
            if (count >= 4) { //check the progress
            	if (SwitchLevel.levelCounter == 3) { //check if user is at last level
            		...
            		Start.closeFrame(); // reset interface
                			...
                			SwitchLevel.newGameProc("Object Oriented programming", SwitchLevel.levelCounter); // create new level
                }
            	else {
            		Start.closeFrame(); // reset interface
            		SwitchLevel.levelCounter = 1;
    ...
    void set(){ 
        ...
        if (current <= 4) {
        	Question question = questionObjOCollections.getQuestion();
